...
2. UI relevance (chắt lọc UI):
   Classify criterion: UI_JOURNEY (thao tác & outcome UI) | BE_ONLY (logic ngầm server/DB/cache → bỏ qua E2E) | UNKNOWN (không invent UI).
   Chỉ sinh E2E TC cho tín hiệu UI_JOURNEY.
3. Traceability (ISTQB): `trace:` → FEATURES|FLOWS|BR|VALIDATION|AC|ERROR|ACTORS|EXEC_CONTEXT|API_UI.
   1 tín hiệu chính / 1 TC — cấm gộp nhiều FR/AC/BR khác nhau trong một TC.
   Cùng `trace:` → tối đa 1 TC (cấm clone wording).
4. Workflow / use-case: BUSINESS_FLOWS = xương sống.
...
6. EP / BVA: VALIDATION có miền/biên trên UI → ≥1 TC lớp tương đương hoặc biên (không nhân bản cùng assert).
7. BR Expansion: mỗi BUSINESS_RULES quan sát được trên UI → ≥1 TC.
8. Scenario Expansion (có tín hiệu mới sinh): Happy · Validation · Permission · Boundary · Error —
   CHỈ nhánh có tín hiệu Output quan sát được trên UI (không checklist giả).
9. Step Expansion: mỗi step BẮT BUỘC dạng `[Hành động] → [Element/nhãn UI] → [Data nếu có]`.
...
11. Coverage gate (không lọt): mọi FEATURES/FLOWS/BR/VALIDATION/AC/ERROR/(ACTORS RBAC)/EXECUTION_CONTEXT
    có item UI_JOURNEY → ≥1 TC. Thiếu lớp có tín hiệu = FAIL (không chấp nhận «đã đủ vì hết quota»).
12. Anti-bloat (siết thừa): bỏ TC nếu (a) trùng trace, (b) cùng flow+cùng expected chỉ đổi title,
    (c) chỉ restates TC khác, (d) không thêm assert/observable mới, (e) BE_ONLY. SUMMARY/NFR/GAPS → không pad.
13. Lock: không đổi/bổ sung requirement ngoài Output.
14. AUTH WHO (khi có tín hiệu): `authRequired` · `authRole` (1 role) · multi → `roles:` + `multiRole: true`.
    Role CHỈ từ ACTORS/EXECUTION_CONTEXT. Không ghi storageState/API-login (HOW=codegen).
15. FEATURE PATH (E2E_GROUNDING): TC sau login / không PUBLIC → `testData` BẮT BUỘC
