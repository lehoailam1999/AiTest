...
function updateStatus(): void {
  if (!statusBar) return;
  if (!bridge) {
    statusBar.text = "$(debug-disconnect) AITest (Antigravity)";
    statusBar.tooltip = "AITest Antigravity bridge stopped";
    return;
  }
  statusBar.text = `$(plug) AITest (Antigravity) :${bridge.port}`;
  statusBar.tooltip = `AITest Antigravity IDE bridge\n${bridge.url}\nclients: ${bridge.clientCount()}\n${bridge.discoveryPath}`;
}

async function startBridge(context: vscode.ExtensionContext): Promise<void> {
  if (bridge) {
    void vscode.window.showInformationMessage(`AITest Antigravity bridge already running on :${bridge.port}`);
    return;
  }
  ...
  updateStatus();
  void vscode.window.setStatusBarMessage(`AITest Antigravity bridge :${bridge.port}`, 3000);
}
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("aitest.showBridgeStatus", async () => {
      if (!bridge) {
        void vscode.window.showWarningMessage("AITest Antigravity bridge is not running.");
        return;
      }
      ...
      void vscode.window.showInformationMessage(
        `AITest Antigravity bridge :${bridge.port} · clients ${bridge.clientCount()} · focus: ${focus}`
      );
    }),
    vscode.commands.registerCommand("aitest.refreshFocus", async () => {
      if (!bridge) {
        void vscode.window.showWarningMessage("Start the AITest Antigravity bridge first.");
        return;
      }
      const snap = await buildFocusSnapshot();
      if (!snap) {
  ...
  if (auto) {
    try {
      ...
      void vscode.window.showErrorMessage(
        `AITest Antigravity bridge failed: ${e instanceof Error ? e.message : String(e)}`
      );
    }
  } else {
    updateStatus();
