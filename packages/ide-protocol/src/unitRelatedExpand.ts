...
export function featureStem(stem: string): string {
  let s = stem.replace(/^(I)(?=[A-Z])/, "");
  // Compound CQRS suffixes first
  s = s.replace(
    /(CommandHandler|QueryHandler|CommandValidator|QueryValidator)$/i,
    ""
  );
  s = s.replace(
    /(Service|Handler|Controller|Validator|UseCase|Policy|Repository|Manager|Provider|Factory|Builder|Options|Config|Settings|Request|Response|Command|Query|Dto|Dtos|Model|Models|Entity|Entities|Enum|Enums|Type|Types|Interface)$/i,
    ""
  );
  return s;
}

function pascalParts(name: string): string[] {
...
export function entryFeatureKeys(entryPathRel: string): string[] {
  const stem = stemOf(entryPathRel);
  const family = featureStem(stem);
  const parts = pascalParts(family || stem);
  const out = new Set<string>();
  if (family && family.length >= 4) out.add(family.toLowerCase());
...
export function isValidationLayerRelatedPath(pathRel: string): boolean {
  const p = norm(pathRel);
  ...
  if (
    /(Dto|Dtos|Request|Response|Command)$/i.test(stem) &&
    !/Handler/i.test(stem)
  ) {
    return true;
  }
 ...
 */
export function relatedShapeBonus(pathRel: string): number {
  ...
  if (
    /(dto|dtos|request|response|contract|model|models)\b/i.test(stem) ||
  ...
  if (
    /(service|handler|validator|usecase|policy)\./i.test(p) ||
...
function featureOverlapScore(
  ...
  for (const k of keys) {
    ...
    if (stem.includes(k) || low.includes(k)) score += 22;
  }
  for (const t of featureTokens) {
    const tl = t.toLowerCase();
    if (tl.length < 4) continue;
    if (stem.includes(tl) || low.includes(tl)) score += 10;
  }
  return score;
}
 ...
 */
export function expandUnitRelatedPaths(opts: ExpandUnitRelatedOpts): string[] {
  const entry = norm(opts.entryPathRel);
  if (!entry) return [];
  const max = Math.max(
    0,
    Math.min(
      opts.maxRelated ?? UNIT_GEN_LIMITS.maxRelatedFiles,
  ...
  const scored: { path: string; score: number; validationLayer: boolean }[] = [];
  for (const raw of opts.allPaths) {
    const pathRel = norm(raw);
    if (!pathRel || pathRel === entry) continue;
    if (pathRel.toLowerCase() === entry.toLowerCase()) continue;
    if (isDeniedUnitRelatedPath(pathRel)) continue;

    const shape = relatedShapeBonus(pathRel);
    const feat = featureOverlapScore(pathRel, keys, featureTokens);
    if (shape < 8 && feat < 22) continue;
    ...
    if (preferDto && validationLayer) {
      ...
      if (/(Dto|Dtos|Validator)$/i.test(stem)) score += 55;
    ...
    if (score < 28 && !(preferDto && validationLayer && feat >= 22)) continue;
  ...
  if (preferDto) {
    ...
    for (const s of dtoFirst) {
      push(s.path);
      if (out.length >= Math.min(3, max)) break;
    }
    for (const s of scored) {
      if (!s.validationLayer) continue;
      push(s.path);
      if (out.length >= Math.min(3, max)) break;
    }
  }
...
export function formatRelatedMarkerLines(relatedPaths: string[]): string[] {
  const cleaned = relatedPaths.map((p) => norm(p)).filter(Boolean);
  if (!cleaned.length) return [];
  if (cleaned.length === 1) return [`related: ${cleaned[0]}`];
