...
import type { SymbolKind, SymbolRef } from "../types.js";
...
function makeId(s: MockSymbol): string {
  return `${s.pathRel}:${s.line}:${s.character}:${s.name}`;
}

function parseSymbolId(id: string): { pathRel: string; line: number; character: number; name: string } | null {
...
export function defaultMockFiles(): MockFile[] {
  return [
    {
...
export function defaultMockSymbols(): MockSymbol[] {
  return [
    {
      name: "AuthService",
      kind: "class",
...
export function mockSearchSymbol(
  symbols: MockSymbol[],
  ...
  const hits: SymbolHit[] = filtered.slice(0, max).map((s) => ({
    id: makeId(s),
    name: s.name,
    kind: s.kind,
    pathRel: s.pathRel,
...
export function mockSearchText(files: MockFile[], params: SearchTextParams): SearchTextResult {
  ...
  const matchesGlob = (pathRel: string, glob?: string): boolean => {
    if (!glob) return true;
    const ext = glob.match(/\.\w[\w.]*/g);
    if (ext?.length) return ext.some((e) => pathRel.toLowerCase().endsWith(e.toLowerCase()));
    return pathRel.toLowerCase().includes(glob.replace(/\*/g, "").toLowerCase());
  };

  for (const f of files) {
    if (!matchesGlob(f.pathRel, params.glob)) continue;
    const lines = f.content.split(/\r?\n/);
    for (let i = 0; i < lines.length; i++) {
      const idx = lines[i].indexOf(q);
      if (idx < 0) continue;
      let preview = lines[i].trim();
      if (preview.length > maxBytes) preview = preview.slice(0, maxBytes);
...
export function mockGoToDefinition(
  symbols: MockSymbol[],
  params: SymbolPositionParams
): GoToDefinitionResult {
  ...
  const hit =
    symbols.find(
      (s) =>
        s.pathRel === pos.pathRel &&
        s.line === pos.line &&
        (pos.name ? s.name === pos.name : true)
    ) || symbols.find((s) => s.name === pos.name);
  if (!hit) {
    return {
      locations: [
        {
...
export function mockFindImplementations(
  symbols: MockSymbol[],
  params: SymbolPositionParams
): FindImplementationsResult {
...
export function mockReadFile(files: MockFile[], params: ReadFileParams): ReadFileResult {
