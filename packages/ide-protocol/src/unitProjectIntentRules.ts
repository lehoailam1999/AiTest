...
import type { UnitIntent, UnitIntentClass } from "./unitIntentAliases.js";
...
export function isWeakBodyRulePattern(pat: string): boolean {
  const t = String(pat || "").trim().toLowerCase();
  if (!t || t.length < 3) return true;
  return UNIT_WEAK_BODY_RULE_PATTERNS.has(t);
}

export function isWeakPathBridgePattern(pat: string): boolean {
  const t = String(pat || "").trim().toLowerCase();
...
export function filterStrongBodyRulePatterns(patterns: string[]): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  for (const raw of patterns || []) {
    const p = String(raw || "").trim();
    if (!p || isWeakBodyRulePattern(p)) continue;
...
export function isSoftGenericRuleHit(hit: string): boolean {
  ...
  const t = raw
    ...
    .toLowerCase();
  if (!t || t.length < 3) return true;
  if (UNIT_SOFT_GENERIC_RULE_HITS.has(t)) return true;
...
export function hasStrongWriteBackSignal(opts: {
  ...
  if (rules.some((h) => !isSoftGenericRuleHit(h))) return true;

  const softHitPayload = (raw: string): boolean => {
    ...
    if (!t || t.length < 3) return true;
    if (isSoftGenericRuleHit(t)) return true;
    return /^(create|update|delete|add|new|throw|badrequest|exception|handle|handler|save|service|command|query|success|call|misc|soft)$/i.test(
      t
    );
  };
  ...
  if (
    hits.some((h) => {
      const s = String(h || "");
      if (/^sutMap/i.test(s)) return true;
      if (/^alias:/i.test(s)) return !softHitPayload(s);
      if (/^(phrase|prefer|techStem):/i.test(s)) return !softHitPayload(s);
      return false;
    })
  ) {
    return true;
  ...
  const prefer = (opts.preferTokens || [])
    ...
    .filter(
      (t) =>
        !/^(create|update|delete|reject|deny|checkcode|check|exists|unique|duplicate|search|validation|trace|input|mock|add|new|code|throw|badrequest|handler|service|command|query|assert|error|success|call|misc|soft)$/i.test(
          t
        )
    );
...
export function parseUnitProjectIntentRules(
  raw: unknown
): UnitProjectIntentRule[] {
  if (!raw || typeof raw !== "object") return [];
  const intents = (raw as UnitProjectIntentRulesFile).intents;
  if (!Array.isArray(intents)) return [];
  return intents.filter(
    (r) => r && typeof r === "object" && typeof r.id === "string" && r.id.trim()
  );
}

function testCue(reSrc: string | undefined, blob: string): boolean {
  const src = (reSrc || "").trim();
  if (!src || !blob) return false;
  try {
 ...
 */
export function applyProjectIntentRules(
  base: UnitIntent,
  rules: UnitProjectIntentRule[] | null | undefined,
  opts: {
    title?: string | null;
    module?: string | null;
    steps?: string | null;
  ...
  const blob = [opts.title, opts.module, opts.steps, opts.expectedResult]
    .filter(Boolean)
    .join("\n");
  const matched: UnitProjectIntentRule[] = [];
  for (const r of rules || []) {
    if (testCue(r.whenTitleOrStepsMatch, blob)) matched.push(r);
  }
  if (!matched.length) {
  ...
  let scopeRefuse: "FAIL_FEATURE_GAP" | undefined;
  const rulePatterns = [...(base.rulePatterns || [])];
  const codePatterns = [...(base.codePatterns || [])];
  const classes = [...base.classes] as UnitIntentClass[];
 ...
 */
export function resolveSutMapPin(
  sutMap: Record<string, string> | null | undefined,
  opts: {
  ...
  if (!sutMap || typeof sutMap !== "object") return null;
  const keys: string[] = [];
  if (opts.preferSutMapKey) keys.push(opts.preferSutMapKey);
  for (const id of opts.matchedIntentIds || []) keys.push(id);
  if (opts.module?.trim()) keys.push(opts.module.trim());
  if (opts.title?.trim()) keys.push(opts.title.trim());

  for (const k of keys) {
    const hit = sutMap[k];
    if (hit?.trim()) return hit.replace(/\\/g, "/").trim();
  }
  ...
  for (const [k, v] of Object.entries(sutMap)) {
    ...
    if (blob.includes(k.toLowerCase())) return v.replace(/\\/g, "/").trim();
  }
  return null;
}
