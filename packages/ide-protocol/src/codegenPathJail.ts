...
export function isAllowedUnitLayoutPath(safeRel: string): boolean {
  ...
  return (
    /\.(csproj|props|targets)$/i.test(after) ||
    /^(jest\.config\.[cm]?js|package\.json|tsconfig(\.[\w-]+)?\.json)$/i.test(after)
  );
}

export function isAllowedE2eEnvKey(key: string): boolean {
  const k = (key || "").trim();
  if (!k) return false;
  if (ALLOWED_E2E_ENV_EXACT.has(k)) return true;
...
export function filterAllowedEnv(
  ...
  for (const [k, v] of Object.entries(env)) {
    ...
    if (k.startsWith("E2E_")) {
      if (isAllowedE2eEnvKey(k)) out[k] = v;
    } else {
      out[k] = v;
    }
  }
