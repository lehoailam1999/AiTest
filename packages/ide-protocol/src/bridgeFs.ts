...
export function defaultBridgeDiscoveryPath(overrideHome?: string): string {
  const home = overrideHome ?? homedir();
  return join(home, IDE_BRIDGE_DIR, IDE_BRIDGE_FILENAME);
}

export function ensureBridgeDir(overrideHome?: string): string {
  const home = overrideHome ?? homedir();
...
export function writeBridgeDiscovery(
...
export function readBridgeDiscovery(filePath?: string): IdeBridgeDiscovery | null {
  const path = filePath ?? defaultBridgeDiscoveryPath();
...
export function clearBridgeDiscovery(filePath?: string): void {
  const path = filePath ?? defaultBridgeDiscoveryPath();
