...
export function extractTcSourceMarkers(text: string): {
  paths: string[];
  codes: string[];
  related: string[];
} {
  ...
  while ((m = codeRe.exec(text || ""))) {
    const c = m[1];
    if (!c) continue;
    if (TC_META_KIND_CODES.has(c.toLowerCase())) continue;
    codes.push(c);
  }
 ...
 */
export function hasUnitSourceMarkers(text: string | null | undefined): boolean {
...
export function isUnitSutResolveSkipped(text: string | null | undefined): boolean {
...
export function significantTokens(text: string): string[] {
  ...
  for (const m of raw.matchAll(/[A-Z][a-z0-9]+(?:[A-Z][a-z0-9]+)*/g)) {
    ...
    if (full.length >= 4 && !STOP.has(full)) bag.add(full);
  }
...
export function sutTcAlignmentScore(opts: {
...
}): { score: number; shared: string[]; markersHit: number } {
  ...
  for (const p of markers.paths) {
    ...
    if (pathsMatchMarker(primaryPath, p)) markersHit += 3;
  }
  for (const c of markers.codes) {
    if (
      codeMatchesPathStem(c, primaryPath) ||
      new RegExp(`\\b${c.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`).test(
        opts.sourceExcerpt || ""
      )
    ) {
      markersHit += 2;
  ...
  for (const t of expanded) {
    ...
    if (STOP.has(t) || DOMAIN_ACTIONISH.has(t)) continue;
    if (pathNorm.includes(t)) aliasPathHits += 1;
  }
  const tcUniq = [...new Set([...significantTokens(opts.tcText), ...expanded])];
  const sut = significantTokens(
  ...
  const uniqStrong = [...new Set(shared)].filter(
    (t) =>
      t.length >= 5 &&
      !STOP.has(t) &&
      !DOMAIN_ACTIONISH.has(t) &&
      !["name", "code", "entity", "success", "result", "value", "item", "list", "dto"].includes(
        t
      )
  );
...
export function unitSutAlignMin(markersHit: number): number {
 ...
 */
export function expectedDomainTokensFromTc(
  tcText: string,
  codeAliases?: Record<string, string[]> | null
): string[] {
  const bag = new Set<string>();
  ...
  for (const t of expandVietnameseToCodeTokens(tcText || "", codeAliases)) {
    const low = t.toLowerCase();
    if (low.length < 4 || DOMAIN_ACTIONISH.has(low) || STOP.has(low)) continue;
    bag.add(low);
    for (const part of t.replace(/([a-z])([A-Z])/g, "$1 $2").split(/\s+/)) {
      const p = part.toLowerCase();
      if (p.length >= 4 && !DOMAIN_ACTIONISH.has(p) && !STOP.has(p)) bag.add(p);
    }
  }
  for (const c of markers.codes) {
    const parts = c.match(/[A-Z][a-z0-9]*/g) || [];
    const head = parts[0]?.toLowerCase();
    if (head && head.length >= 4 && !DOMAIN_ACTIONISH.has(head)) bag.add(head);
  }
  for (const p of markers.paths) {
    const norm = p.replace(/\\/g, "/");
    const m = norm.match(/\/(?:commands|handlers|controllers|services|domain)\/([^/]+)\//i);
    if (m?.[1] && m[1].length >= 3) bag.add(m[1].toLowerCase());
 ...
 */
export function extractPathDomainHints(primaryPath: string): string[] {
  const norm = (primaryPath || "").replace(/\\/g, "/");
  const bag = new Set<string>();
  const folder = norm.match(
    /\/(?:commands|handlers|controllers|services|domain|features|modules)\/([^/]+)\//i
  );
  if (folder?.[1] && folder[1].length >= 3) bag.add(folder[1].toLowerCase());
  const base = norm.split("/").pop()?.replace(/\.[^.]+$/, "") || "";
  const parts = base.match(/[A-Z][a-z0-9]*/g) || [];
  const head = parts[0];
...
export function detectCodeStack(code: string): UnitCodeStack {
  const c = code || "";
  if (
    /^\s*using\s+[\w.]+;/m.test(c) ||
  ...
  if (
    /\bdescribe\s*\(|\bit\s*\(|\btest\s*\(|from\s+['"]@angular|from\s+['"]@nestjs|vitest|jest/.test(
      c
    ) ||
  ...
  if (/\bdef\s+\w+\(|\bimport\s+\w+|pytest|unittest/.test(c)) return "python";
  if (/\bpackage\s+\w+|@Test\b|org\.junit/.test(c)) return "java";
  if (/\bfunc\s+\w+\(|package\s+\w+/.test(c) && /\.go\b/.test(c)) return "go";
  return "unknown";
}

export function stackForPath(relPath: string): UnitCodeStack | "any" {
...
export function assertStackMatchesPath(relPath: string, code: string): void {
  ...
  if (got === "unknown") {
    ...
    if (want === "csharp" && /describe\s*\(|import\s+\{/.test(code)) {
      throw new Error(
        `Stack mismatch: path ${relPath} is C# but content looks TypeScript/Jest`
      );
    }
    if (
      (want === "typescript" || want === "javascript") &&
      /^\s*using\s+[\w.]+;/m.test(code)
    ) {
      throw new Error(
        `Stack mismatch: path ${relPath} is JS/TS but content looks C#`
      );
    }
    return;
  }
  if (got !== want) {
    // js/ts soft alias
    if (
      (want === "typescript" && got === "javascript") ||
      (want === "javascript" && got === "typescript")
    ) {
      return;
    }
    throw new Error(
      `Stack mismatch: path expects ${want} (${relPath}) but generated ${got}`
    );
  }
}
 ...
 */
export function findInventedRuleSmells(code: string, sutExcerpt: string): string[] {
  const smells: string[] = [];
  ...
  if (
    /\b(?:AllowedExtensions|AllowedMimeTypes|ValidExtensions)\s*=\s*new\s*(?:\[|List|HashSet)/i.test(
      c
    ) &&
  ...
  if (
    /\b(?:MAX_LENGTH|MaxLength)\s*=\s*\d{2,4}\b/.test(c) &&
  ...
  if (
    /Assert\.(True|False|Equal)\([^)]{0,80}Length\s*[><=]+\s*\d{2,4}/.test(c) &&
 ...
 */
export function findNonPortableTestHarnessSmells(code: string): string[] {
  const smells: string[] = [];
  const c = code || "";
...
export function assertUnitGenQuality(opts: {
  ...
  const sutName = (opts.primaryPath.split("/").pop() || "")
    ...
    .toLowerCase();
  const mentionsSut =
    opts.code.replace(/\\/g, "/").includes(opts.primaryPath.replace(/\\/g, "/")) ||
  ...
  if (!mentionsSut && align.score < postMin) {
    throw new Error(
      `Generated test does not ground on resolved SUT «${opts.primaryPath}» (alignment=${align.score})`
    );
  }
...
export const CSHARP_USING_TO_PACKAGE: Record<string, string> = {
...
export function detectCsharpPackagesFromTestCode(code: string): string[] {
  ...
  for (const m of c.matchAll(/^\s*using\s+([\w.]+)\s*;/gm)) {
    ...
    for (const [prefix, pkg] of Object.entries(CSHARP_USING_TO_PACKAGE)) {
      if (ns === prefix || ns.startsWith(prefix + ".")) found.add(pkg);
    }
  }
  if (/\bFluentAssertions\b|\.Should\(\)/.test(c)) found.add("FluentAssertions");
