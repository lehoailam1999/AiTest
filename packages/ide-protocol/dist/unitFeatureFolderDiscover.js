import { filterStrongRankTokens, isWeakUnitRankToken, UNIT_WEAK_RANK_TOKENS, } from "./unitSutGate.js";
/**
...
export function isCrossCuttingSoftPrimaryPath(pathRel) {
 ...
 */
export function softCrossCuttingDenied(pathRel, gateOrPreferTokens) {
    if (!isCrossCuttingSoftPrimaryPath(pathRel))
        return false;
    ...
    const pathInfra = UNIT_INFRA_PATH_TOKENS.map((t) => t.toLowerCase().replace(/[^a-z0-9]/g, "")).filter((s) => s.length >= 4 && p.includes(s));
    if (!pathInfra.length)
        return true;
    return !toks.some((t) => pathInfra.some((s) => t.includes(s) || s.includes(t)));
}
/**
 * Signed/presigned URL or access-token generators — not domain create/validate.
 */
export function isSignedUrlOrTokenGeneratePath(pathRel) {
    const base = (pathRel || "").replace(/\\/g, "/").split("/").pop() || String(pathRel || "");
    if (!base)
        return false;
    if (/(Presigned|PreSigned|SignedUrl|SignedLink)/i.test(base))
        return true;
    if (/^(Generate|Create|Request)[A-Za-z0-9]*(Url|Link|Token)(Command)?Handler\b/i.test(base)) {
        return true;
    }
    return false;
}
...
function normTitleBlob(titleTokens) {
 ...
 */
export function titleCueBridgeStems(titleTokens) {
    const { blob, compact } = normTitleBlob(titleTokens);
    const out = [];
    if (/checkcode|kiemtra|validator|maxlength|alreadyexist|duplicate|trungma|unique/.test(compact) ||
    ...
    if (!/khong\s*phai\s*ky\s*thuat/.test(blob) &&
        (/\b(thiet\s*bi|imei|device)\b/.test(blob) || /thietbi|\bimei\b/.test(compact))) {
        out.push("Device");
    }
    if (/hinhanh|tailen|initupload/.test(compact) ||
        /\b(hinh\s*anh|tai\s*len|upload|image)\b/.test(blob)) {
        out.push("InitUpload", "Image");
    }
    const hasFamily = out.some((s) => /^(Assign|Person|Device|InitUpload|Image|IsOccupied)$/i.test(s));
    if ((/taomoi|themmoi|\bcreate\b/.test(compact) ||
...
export function titleDiscoverKinds(titleTokens) {
    ...
    return {
        ...
        create: /taomoi|themmoi|\bcreate\b/.test(compact) ||
            /\b(tao\s*moi|them\s*moi|create)\b/.test(blob),
 ...
 */
export function featureFolderSegmentFromPath(pathRel) {
    const p = normPath(pathRel);
    const m = p.match(/\/(?:Commands|Queries|QueryHandlers|CommandHandlers|Handlers|Features|Modules|UseCases)\/([^/]+)\//i);
    if (!m?.[1])
        return null;
    const seg = m[1];
    if (seg.length < 3)
        return null;
    if (WEAK_FOLDER_SEGMENTS.has(seg.toLowerCase()))
        return null;
    return seg;
}
function isUbiquitousPathToken(tok, paths) {
    const tl = tok.toLowerCase();
    if (tl.length < 4 || !paths.length)
        return false;
...
function textHasBridge(hay, bridge) {
...
export function bridgeNeedlesForDiscover(intent, titleTokens = []) {
    const fromIntent = [...(intent?.rulePatterns || [])]
        ...
        .filter((p) => !/^(throw|validate|exists|any|generate|exceed|badrequest|argumentexception)$/i.test(p));
    const titleLow = titleTokens.map((t) => t.toLowerCase());
    const fromTitleBridges = TITLE_BRIDGE_STEMS.filter((s) => {
        const sl = s.toLowerCase();
        return titleLow.some((t) => t === sl || (t.length >= sl.length && t.includes(sl)));
    });
    const fromTitleCues = titleCueBridgeStems(titleTokens);
    return filterStrongRankTokens(uniq([...fromIntent, ...fromTitleBridges, ...fromTitleCues]));
}
/**
 ...
 */
function narrowAssignBridgesForDossier(bridges, titleTokens) {
    const { blob } = normTitleBlob(titleTokens);
    if (!/\b(ho\s*so|vu\s*an|dossier)\b/.test(blob))
        return bridges;
    const dossier = bridges.filter((p) => /Assign\w*Case|Case\w*Assign/i.test(p));
 ...
 */
function narrowBridgesByActiveNeedles(bridges, needles) {
    const has = (n) => needles.some((x) => x.toLowerCase() === n);
    let out = bridges;
    if (has("person")) {
        const hit = out.filter((p) => /Person/i.test(p) && !/PersonImage|ImagePerson/i.test(p));
        if (hit.length)
            out = hit;
    }
    if (has("device")) {
    ...
    if (has("image") || has("initupload")) {
        const hit = out.filter((p) => /InitUpload/i.test(p) ||
 ...
 */
export function discoverFeatureFoldersFromIndex(index, allPaths, opts) {
    const paths = uniq([
    ...
    const bridgePaths = [];
    const addBridge = (pathRel) => {
        const np = normPath(pathRel);
        if (!np || isSpaOrComponentPath(np) || bridgePaths.includes(np))
            return;
        bridgePaths.push(np);
    };
    for (const pathRel of paths) {
        if (isSpaOrComponentPath(pathRel))
            continue;
        ...
        if (needles.some((n) => textHasBridge(hay, n)))
            addBridge(pathRel);
    }
    ...
    for (const n of needles) {
        ...
        for (const [sym, plist] of Object.entries(symIndex)) {
            if (!textHasBridge(sym, n) && sym !== key)
                continue;
    ...
    const uploadActive = needles.some((n) => /^(InitUpload|Image|MaxFileSize|FileSize)$/i.test(n));
    const folderVotes = new Map();
    const vote = (tok, weight) => {
        if (!tok || tok.length < 3)
            return;
        if (IT_NEEDLE_SET.has(tok.toLowerCase()))
            return;
        if (isWeakUnitRankToken(tok) && !(uploadActive && /^Upload$/i.test(tok))) {
            return;
        }
        if (WEAK_FOLDER_SEGMENTS.has(tok.toLowerCase()))
            return;
        if (/^(assign|unassign|case|dialog|controller|overview|report|home|record|digital|devices)$/i.test(tok)) {
            return;
        }
        if (isUbiquitousPathToken(tok, paths))
            return;
        if (/Test$/i.test(tok) || /HandlersTest/i.test(tok))
            return;
    ...
    for (const p of bridges.slice(0, 64)) {
        ...
        if (/\/Services\//i.test(p)) {
            for (const tok of pathSegmentTokens(p))
                vote(tok, 1);
        }
    }
...
export function isInfraCrossCuttingPath(pathRel) {
    const p = normPath(pathRel);
    if (UNIT_INFRA_PATH_TOKENS.some((t) => p.toLowerCase().includes(t.toLowerCase()))) {
        return true;
    }
    return isSignedUrlOrTokenGeneratePath(p);
}
export function infraPathDemoteScore(pathRel, intent) {
    const behavior = Boolean(intent?.requiresBodyRule) ||
    ...
    if (!behavior || !isInfraCrossCuttingPath(pathRel))
        return 0;
    return 40;
}
export function filterCandidatesByFeatureFolders(candidates, featureTokens) {
    const toks = filterStrongRankTokens(featureTokens);
    if (featureTokens.some((t) => /^Upload$/i.test(t)) &&
    ...
    const hit = candidates.filter((c) => toks.some((t) => normPath(c.pathRel).toLowerCase().includes(t.toLowerCase())));
    if (!hit.length)
        return { candidates: [], filtered: true };
