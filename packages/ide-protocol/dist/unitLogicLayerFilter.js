 ...
 */
export function isWeakUnitClientPath(pathRel) {
    const p = norm(pathRel);
    if (!p)
        return true;
    if (UNIT_RANK_POLICY.weakClientAppAdminRe.test(p))
        return true;
    if (UNIT_RANK_POLICY.weakHttpClientRe.test(p))
        return true;
    if (/\/clientapp\/|\/client-app\/|\/wwwroot\//.test(p))
        return true;
    return false;
}
/** True when TC path: marker explicitly points at this file (manual override). */
export function markersPointAtPath(pathRel, markers) {
    if (!pathRel || !markers?.paths?.length)
        return false;
    for (const m of markers.paths) {
        if (pathsMatchMarker(pathRel, m))
            return true;
    }
    return false;
}
 ...
 */
export function isBlockedUnitPrimaryPath(pathRel, markers) {
    if (markersPointAtPath(pathRel, markers))
        return false;
 ...
 */
export function isDeniedUnitPrimaryPath(pathRel) {
    const p = norm(pathRel);
    if (!p)
        return true;
    // Existing tests / suites / generated
    if (/(^|\/)(test|tests|__tests__|spec)(\/|$)/.test(p))
        return true;
    if (/tests?\.cs$/.test(p) || /\.(tests?|spec)\.cs$/.test(p))
        return true;
    if (/\.(spec|test)\.(ts|tsx|js|jsx)$/.test(p))
        return true;
    if (/\/aitest\/|\/\.ai-test\//.test(p))
        return true;
    if (/\/e2e\/|\.e2e\.|\/playwright/.test(p))
        return true;
    if (/\.page\.(ts|tsx|js|jsx)$/.test(p))
        return true;
    if (/\/migrations?\//.test(p) || /\.designer\.cs$|\.snapshot\.cs$|modelsnapshot\.cs$/.test(p))
        return true;
    // FE / SPA shells + thin HTTP upload clients
    if (isWeakUnitClientPath(pathRel))
        return true;
    if (/\.component\.(ts|tsx|js|jsx)$/.test(p))
        return true;
    if (/\.(tsx|jsx|vue|html|cshtml|razor)$/.test(p))
        return true;
    // Constants / pipes / FE-only helpers
    if (/\.(constant|constants|enum|enums|pipe|directive|interceptor|guard)\.(ts|js)$/.test(p)) {
        return true;
    }
 ...
 */
export function isAnemicEntityLikePath(pathRel) {
    const p = norm(pathRel);
    if (!p)
        return false;
    ...
    if (/\/(entities?|models?|dto|dtos|viewmodels?|pocos?)\//.test(p))
        return true;
    if (/\/domain\/entities?\//.test(p))
        return true;
    // Bare *Entity.cs / *Model.cs / *Dto(s).cs without handler/service stem
    const base = p.split("/").pop() || "";
    if (/\.(entity|model|dto|poco)\.(cs|ts)$/i.test(base))
        return true;
    if (/entity\.cs$/i.test(base) || /model\.cs$/i.test(base))
        return true;
    if (/dtos?\.(cs|ts)$/i.test(base))
        return true;
    if (/(request|response|viewmodel)s?\.(cs|ts)$/i.test(base))
        return true;
    return false;
}
/**
 ...
 */
export function tcImpliesBehaviorPrimary(tcText) {
    const t = tcText || "";
    return (/từ\s*chối|tu\s*choi|\breject(ed|ion)?\b|\bdeny\b|\bdenied\b/i.test(t) ||
 ...
 */
export function isPreferredLogicLayerPath(pathRel) {
    if (isDeniedUnitPrimaryPath(pathRel))
        return false;
    if (isAnemicEntityLikePath(pathRel))
        return false;
    const p = norm(pathRel);
    const base = p.split("/").pop() || p;
    if (/\.(service|handler|validator|usecase|use-case|policy|command|query)\./.test(p)) {
        return true;
    }
    ...
    if (/infrastructure\/services?\//.test(p))
        return true;
    if (/\/application\//.test(p))
        return true;
    ...
    const preferSeg = new Set(UNIT_RANK_POLICY.preferPathShapes.map((s) => s.toLowerCase().replace(/-/g, "")));
    for (const part of parts) {
        const flat = part.replace(/-/g, "");
        if (preferSeg.has(flat))
            return true;
    }
    return false;
}
 ...
 */
export function isDeniedUnitRelatedPath(pathRel) {
    const p = norm(pathRel);
    if (!p)
        return true;
    if (/(^|\/)(test|tests|__tests__|spec)(\/|$)/.test(p))
 ...
 */
export function filterUnitLogicLayerPaths(paths) {
 ...
 */
export function filterUnitLogicLayerCandidates(cands, opts) {
