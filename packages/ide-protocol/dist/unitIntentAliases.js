 ...
 */
import { expandVietnameseToCodeTokens, normalizeAliasKey, } from "./viCodeAliases.js";
import { UNIT_INTENT_DEFS, UNIT_UI_MASTER_STRONG_RE, } from "./loadUnitIntentDefs.js";
...
export function unitIntentBlob(tc, requirementTitle) {
 ...
 */
export function unitIntentFunctionBlob(tc, opts) {
 ...
 */
export function unitIntentGroundingBlob(tc, requirementTitle) {
...
export function unitIntentTitleModuleBlob(tc, requirementTitle) {
    return unitIntentGroundingBlob(tc, requirementTitle);
}
/** Highest-priority matched class (for gate/debug). */
export function primaryIntentClass(classes) {
    let best;
...
export function hasStrongUiMasterCues(blob) {
 ...
 */
export function extractUnitIntent(tc, opts) {
    ...
    const uiBlob = opts?.uiFromTitleModuleOnly !== false
        ? unitIntentTitleModuleBlob(tc, opts?.requirementTitle)
        : blob;
    const classes = [];
    const codePatterns = [];
    const rulePatterns = [];
    const classFeatureTokens = [];
    const featureTokens = [];
    ...
    if (blob || functionBlob) {
        for (const def of UNIT_INTENT_DEFS) {
            ...
            const matchBlob = def.id === "ui_master_create"
                ? uiBlob
                : def.id === "persist_create" || def.id === "persist_update"
                    ? functionBlob
                    : blob;
            if (!matchBlob || !def.cues.some((re) => re.test(matchBlob)))
                continue;
            classes.push(def.id);
    ...
    const aliasSource = [
        ...
        .join(" ");
    if (aliasSource) {
        featureTokens.push(...expandVietnameseToCodeTokens(aliasSource, opts?.projectAliases));
    }
    // Size-limit + reject reinforce validation body-rule.
    if (classes.includes("upload_size_limit") &&
    ...
    let uiOnly = classes.includes("ui_master_create") && hasStrongUiMasterCues(uiBlob);
