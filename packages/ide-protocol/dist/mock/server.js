...
import { isRequest, makeError, makeNotification, makeSuccess, parseJsonRpcMessage, RpcErrorCode, } from "../rpc.js";
import { IdeMethods, IdeNotifications } from "../methods.js";
import { IDE_PROTOCOL_VERSION } from "../constants.js";
import { EXTENSION_CAPABILITIES } from "../capabilities.js";
import { clearBridgeDiscovery, createDiscovery, writeBridgeDiscovery, } from "../bridgeFs.js";
import { join } from "node:path";
import { tmpdir } from "node:os";
import { defaultMockFiles, defaultMockSymbols, mockFindImplementations, mockFindReferences, mockGoToDefinition, mockReadFile, mockSearchSymbol, mockSearchText, } from "./commandCatalog.js";
function defaultPacket(workspaceRoot) {
    return {
        protocolVersion: 1,
        ide: "mock",
        workspaceRoot,
        ...
        focusSnippet: "export class AuthService {\n  constructor(private users: UserRepository) {}\n  async login() {}\n}\n",
        diagnostics: { confidence: "high", gaps: [] },
    };
}
export async function startMockIdeServer(opts = {}) {
    ...
    const discoveryPath = opts.discoveryPath ?? join(tmpdir(), "aitest-p0-mock", `ide-bridge-${Date.now()}.json`);
    ...
    wss.on("connection", (socket) => {
        ...
        socket.on("message", (buf) => {
            ...
            if (!isRequest(msg))
                return;
            const reply = (body) => socket.send(JSON.stringify(body));
    ...
    return {
        ...
        notifyFocusChanged: (params) => {
            ...
            for (const c of clients) {
                if (authed.has(c) && c.readyState === WebSocket.OPEN)
                    c.send(body);
            }
        },
        close: async () => {
