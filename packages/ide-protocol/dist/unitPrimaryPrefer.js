 ...
 */
export function isInterfaceLikePrimaryPath(pathRel) {
    const p = normLow(pathRel);
    if (!p)
        return false;
    if (/\/interfaces?\//.test(p) || /\.interface\./i.test(p))
        return true;
...
export function stripInterfaceStemPrefix(stem) {
    const s = (stem || "").trim();
    if (/^I[A-Z]/.test(s))
        return s.slice(1);
    return s;
 ...
 */
export function pathsMatchMarker(candidatePath, markerPath) {
    ...
    if (candBase && wantBase && candBase === wantBase)
        return true;
    ...
    if (want.includes("/") && cand.endsWith(want)) {
        ...
        if (candStem.toLowerCase() === wantStem.toLowerCase())
            return true;
    }
    return false;
}
 ...
 */
export function codeMatchesPathStem(code, pathRel) {
    const c = (code || "").trim();
    if (!c)
        return false;
    const stem = stemOfPath(pathRel);
...
export function implementationFamilyStem(pathRel) {
    const stem = stemOfPath(pathRel);
    const stripped = stripInterfaceStemPrefix(stem);
    return stripped
        .replace(/(Service|Handler|Controller|Validator|UseCase|Policy|Repository|Manager|Provider)$/i, "")
        .toLowerCase();
}
/** Prefer Handler/Service over bare Command / Delete when picking among concretes. */
function preferConcreteUnitShape(paths) {
    const scored = paths.map((p) => {
        const base = stemOfPath(p);
        let rank = 50;
        if (/Handler$|Service$/i.test(base))
            rank = 100;
        else if (/Delete/i.test(base))
            rank = 15;
        else if (/Command$/i.test(base) && !/Handler$/i.test(base))
            rank = 40;
        else if (/Query$/i.test(base))
            rank = 30;
        return { p, rank };
    });
 ...
 */
export function preferImplementationOverInterface(paths) {
 ...
 */
export function promoteImplementationPrimary(primaryPath, pool) {
    const primary = norm(primaryPath);
    if (!primary || !isInterfaceLikePrimaryPath(primary))
        return primary;
    ...
    for (const p of pool) {
        const n = norm(p);
        if (!n || isInterfaceLikePrimaryPath(n))
            continue;
        if (implementationFamilyStem(n) === fam)
            return n;
    }
    return primary;
}
/** True when primary matches at least one path: or code: marker (exact helpers). */
export function primaryMatchesMarkers(primaryPath, markers) {
    const primary = norm(primaryPath);
    if (!primary)
        return false;
    for (const p of markers.paths || []) {
        if (pathsMatchMarker(primary, p))
            return true;
    }
    for (const c of markers.codes || []) {
        if (codeMatchesPathStem(c, primary))
            return true;
    }
    return false;
}
