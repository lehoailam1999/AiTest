...
export const UNIT_GEN_SCOPE_DEFAULT = "backend";
...
export function isWeakUnitRankToken(token) {
...
export function filterStrongRankTokens(tokens) {
    const seen = new Set();
    const out = [];
    for (const raw of tokens || []) {
        const t = String(raw || "").trim();
        if (!t || isWeakUnitRankToken(t))
            continue;
 ...
 */
const FEATURE_GAP_CHECKS = [
    {
        label: "malware/antivirus",
        // Require real scenario cues — not the word "malware" inside unit-conventions examples.
        tcRe: /\bmalware\b|\bantivirus\b|\banti[\s-]?virus\b|\bmã\s*độc\b|\bma[\s-]+doc\b|\bquét\s*mã\s*độc\b|\bvirus\s*scan\b/i,
    ...
    },
    {
        ...
        excerptRe: /unique|duplicate|exists|already|trùng|Duplicate|IsUnique|AnyAsync|FirstOrDefault|conflict|Conflict/i,
    },
    {
    ...
    },
    {
        ...
        tcRe: /từ\s*chối|tu\s*choi|\breject(ed|ion)?\b|\bdeny\b|không\s*cho\s*phép|khong\s*cho\s*phep|bị\s*từ/i,
        // Property name alone (IsOccupied) is not reject logic — need throw/guard
        excerptRe: /throw|ArgumentException|BadRequest|InvalidOperation|Reject|Deny|Forbidden|return\s+false|Can[A-Z]|Validate|if\s*\([^)]*IsOccupied|IsOccupied\s*==\s*true/i,
    },
    {
        label: "enable-disable-state",
        tcRe: /\bdisable\b|\benable\b|ngăn\s*trống|ngan\s*trong|đã\s*chứa|da\s*chua/i,
        excerptRe: /IsOccupied|Enable|Disable|CanSelect|IsEnabled|IsDisabled|filter|Where\(|Any\(/i,
    },
];
/**
 ...
 */
export function scenarioTextForFeatureGap(tcText) {
    let t = tcText || "";
    t = t.replace(/##\s*Project rules[\s\S]*?(?=##\s*(?:Approved|Request|Source|Related)|\z)/gi, "\n");
    t = t.replace(/#\s*Unit test conventions[\s\S]*?(?=##\s|\z)/gi, "\n");
    t = t
        .split("\n")
        .filter((line) => !/^\s*#\s*auto-enriched\b/i.test(line) &&
        !/^\s*#\s*field-resolve\s*:/i.test(line) &&
        !/^\s*#\s*sut-resolve\b/i.test(line) &&
        !/BR\s*\/\s*malware|malware\s*\/\s*validation|e\.g\.\s*antivirus|FAIL_FEATURE_GAP|authoritative policy SoT/i.test(line))
        .join("\n");
    return t;
}
function extractConstraint(tcText) {
 ...
 */
export function constraintFamily(constraint) {
    const c = (constraint || "").trim().toLowerCase();
    if (!c)
        return null;
    if (/unique|duplicate|trùng|đã\s*tồn\s*tại|already\s*exist/.test(c))
        return "unique";
    if (/maxlength|max\s*length|\d+\s*ký|giới\s*hạn\s*tối\s*đa/.test(c))
        return "maxlength";
    if (/required|bắt buộc|not\s*empty|không.*trống|mandatory/.test(c))
        return "required";
    return "other";
}
export function detectFeatureGap(tcText, sourceExcerpt) {
    ...
    if (family === "maxlength") {
        const hasMax = /\[\s*maxlength\s*\(|\[\s*stringlength\s*\(|lengthattribute|@size\s*\(|@length\s*\(|maxlength\s*[:=]|\.max\s*\(/i.test(ex);
        return hasMax ? { gap: false } : { gap: true, label: "maxlength" };
    }
    if (family === "unique") {
        const hasDup = /unique|duplicate|exists|already|trùng|Duplicate|IsUnique|AnyAsync|FirstOrDefault|conflict|Conflict/i.test(ex);
...
export function decideUnitSutGate(opts) {
    ...
    const markersHit = opts.markersHit != null
        ? opts.markersHit
        : markers.paths.length + markers.codes.length;
    const scope = opts.unitScope || UNIT_GEN_SCOPE_DEFAULT;
    const floor = Math.max(0, opts.minAlignment != null && Number.isFinite(opts.minAlignment)
    ...
    const block = (partial) => ({ ...partial, ...intentMeta });
    ...
    if (scope === "backend" &&
        ...
        return block({
            ...
            reason: `Intent «${intent.primaryClass || "ui"}» is UI/master/form — ` +
    ...
    if (hasMarkers && !primaryMatchesMarkers(primary, markers)) {
        return block({
            ...
            reason: `Primary «${primary}» does not match Test Data path:/code: ` +
    ...
    if (intent.requiresBodyRule && intent.rulePatterns.length && fullExcerpt) {
        ...
        if (!hitRule) {
            return block({
                decision: "block",
                code: "FAIL_FEATURE_GAP",
                reason: `Intent «${intent.primaryClass}» requires body rule ` +
...
export function isUnitGenRefuseOutput(code) {
 ...
 */
export function weakCommonPathTokenPenalty(pathRel, opts) {
    if (opts?.hasMarkers || opts?.strongTokenHit)
        return 0;
    const base = pathRel
        .replace(/\\/g, "/")
        .split("/")
    ...
    if (weak.length >= Math.ceil(lows.length * 0.6))
        return -18;
    if (weak.includes("create") || weak.includes("upload") || weak.includes("unit"))
        return -8;
    return 0;
}
